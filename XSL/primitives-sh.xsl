        ...
        <xsl:choose>
          ...
          <xsl:when test="@modifier and not(xgf:modifier)">
            <xsl:call-template name="simple-command">
              <xsl:with-param name="cmd" select="@nm"/>
          ...
          <xsl:otherwise>
            <xsl:call-template name="simple-command">
              ...
              <xsl:with-param name="modifier">
                <xsl:choose>
                  <xsl:when test="@nm='MIRP' or @nm='MDRP'">
                    <!--
                        defaults are:
                        set-rp0: true
                        min-distance: true
                  ...
                  <xsl:when test="@nm='ROUND' or @nm='NROUND'">
                    <!-- default is gray -->
                  ...
                  <xsl:when test="@nm='SDPVTL' or @nm='SPVTL' or @nm='SFVTL'">
                    <!-- default is parallel -->
                    <xsl:call-template name="to-line-bit">
                      <xsl:with-param name="tlb">
                        <xsl:choose>
                          <xsl:when test="xgf:modifier[@type='to-line']">
                            <xsl:value-of select="xgf:modifier[@type='to-line']/@val"/>
                          </xsl:when>
                          <xsl:otherwise>
                  ...
                  <xsl:when test="@nm='SVTCA' or @nm='SPVTCA'
                    or @nm='SFVTCA' or @nm='IUP'">
                    <!-- default is x axis -->
                    <xsl:call-template name="axis-bit">
                      <xsl:with-param name="axis">
                        <xsl:choose>
                          <xsl:when test="xgf:modifier[@type='axis']">
                            <xsl:value-of select="xgf:modifier[@type='axis']/@val"/>
                          </xsl:when>
                          <xsl:otherwise>
                            <xsl:text>x</xsl:text>
                          </xsl:otherwise>
                  ...
                  <xsl:when test="@nm='MIAP' or @nm='MDAP'">
                    <!-- default is yes -->
                    ...
                    <xsl:call-template name="grid-fitted-bit">
                      <xsl:with-param name="grid-fitted"
                        select="boolean(not(xgf:modifier[@type='grid-fitted']) or
                                xgf:modifier[@type='grid-fitted']/@val='yes')"/>
                    </xsl:call-template>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:call-template name="error-message">
