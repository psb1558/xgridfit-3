...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--
      ...
      PUSH command is output according to the size pm, or if none,
  ...
  <xsl:template match="xgf:gl" mode="resolve-local-value">
    <xsl:param name="val"/>
    ...
    <xsl:call-template name="expression">
      ...
      <xsl:with-param name="called-from"
		      select="'gl with mode resolve-local-value'"/>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template name="expression-with-offset">
    <xsl:param name="val"/>
    <xsl:param name="this-gl" select="ancestor::xgf:gl"/>
    <xsl:param name="add-mode" select="false()"/>
...
-->
	...
	<xsl:variable name="v">
	  <xsl:call-template name="expression">
	    ...
	    <xsl:with-param name="this-gl" select="$this-gl"/>
	    <xsl:with-param name="permitted" select="$permitted"/>
  ...
  <!--
      ...
      lookups properly by scope: e.g. a locally declared var is
      looked up before a global cn. All number lookups go
  ...
  <xsl:template name="expression">
    ...
    <xsl:param name="this-gl" select="ancestor-or-self::xgf:gl"/>
    <xsl:param name="this-fn" select="ancestor::xgf:fn"/>
    <xsl:param name="this-prep" select="ancestor::xgf:prep"/>
    <xsl:param name="mp-container"/>
    <xsl:param name="all-fn-params" select="$this-fn/xgf:pm"/>
    <xsl:param name="all-variables"
	       select="$this-gl/xgf:var|$this-fn/xgf:var|
		       $this-prep/xgf:var"/>
    <xsl:param name="all-global-variables" select="/xgf:xgridfit/xgf:var"/>
    <xsl:param name="all-mo-params"
	       select="ancestor::xgf:mo/xgf:pm|
		       ancestor::xgf:gl/xgf:pm"/>
    <xsl:param name="all-constants" select="$this-gl/xgf:cn"/>
    <xsl:param name="all-global-constants" select="/xgf:xgridfit/xgf:cn"/>
    <xsl:param name="all-aliases"
	       select="$this-gl/xgf:alias|ancestor::xgf:fn/xgf:alias|
		       ancestor::xgf:mo/xgf:alias|
		       ancestor::xgf:prep/xgf:alias"/>
...
-->
    ...
    <xsl:choose>
      <xsl:when test="$all-aliases[@nm=$valn]">
	<xsl:message><xsl:text>$all-aliases</xsl:text></xsl:message>
	<xsl:call-template name="expression">
	  <xsl:with-param name="val" select="$all-aliases[@nm=$valn]/@target"/>
	  <xsl:with-param name="permitted" select="$permitted"/>
	  ...
	  <xsl:with-param name="mp-container" select="$mp-container"/>
	  <xsl:with-param name="all-fn-params" select="$all-fn-params"/>
      ...
      <xsl:when test="$all-global-aliases[@nm=$valn]">
	<xsl:message><xsl:text>$all-global-aliases</xsl:text></xsl:message>
	<xsl:call-template name="expression">
	  <xsl:with-param name="val" select="$all-global-aliases[@nm=$valn]/@target"/>
	  <xsl:with-param name="permitted" select="$permitted"/>
	  <xsl:with-param name="cvt-mode" select="$cvt-mode"/>
	  <xsl:with-param name="force-to-index" select="$force-to-index"/>
	  <xsl:with-param name="need-number-now" select="$need-number-now"/>
      ...
      <!--
	  If we're in a mo and this is a mo parameter, recurse
	  substituting the value. I don't think this will work for a
	  callm inside a mo. This would be a very desirable
	...
	<xsl:choose>
	  ...
	  <xsl:when test="string-length($this-id) and
		...
		mode="get-my-val">
	      ...
	      <xsl:with-param name="expect" select="$expect"/>
	      <xsl:with-param name="need-number-now" select="$need-number-now"/>
	      <xsl:with-param name="called-from" select="'recurse for pm'"/>
	  ...
	  <!--
	      The second case is that there is a default @val for
	      this pm. In that case we simply recurse as if it
	      were an alias.
	  -->
	  ...
	  <!-- <xsl:when test="$is-mo"> -->
		...
		<xsl:call-template name="get-remaining-mp-id">
	      ...
	      <xsl:with-param name="all-global-aliases" select="$all-global-aliases"/>
	      <xsl:with-param name="debug" select="$debug"/>
	      <xsl:with-param name="called-from" select="'recurse for pm default val'"/>
	      <xsl:with-param name="recursion" select="$recursion + 1"/>
	      <xsl:with-param name="to-stack" select="$to-stack"/>
	      <xsl:with-param name="add-mode" select="$add-mode"/>
	      <xsl:with-param name="expect" select="$expect"/>
	  ...
	  <!--
	      It's an error if there's no @val on either the
	      <wpm> or the <pm>.
	  -->
	  <xsl:otherwise>
	    <xsl:message>
	...
	<!--
	    This var optimizes a bit for programs in which
	    constants declared in the gl element are very common.
	-->
	<xsl:variable name="v-name" select="$all-constants[@nm=$valn]/@val"/>
	...
	<!--
	    Do all the work of resolving the val in this var, and
	...
	<xsl:variable name="resolved-val">
	  <xsl:choose>
		...
		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	    -->
	    <!-- It's a number literal with a decimal pt, understood to
...
-->
		...
		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	    -->
	    <!-- It's a locally declared cn. If it's an integer, just return it.
...
-->
		...
		<xsl:otherwise>
		  <xsl:call-template name="expression">
		    ...
		    <xsl:with-param name="mp-container" select="$mp-container"/>
		    <xsl:with-param name="debug" select="$debug"/>
		    <xsl:with-param name="called-from" select="'exp: recursing to resolve cn'"/>
		    <xsl:with-param name="recursion" select="$recursion + 1"/>
		  </xsl:call-template>
		</xsl:otherwise>
	      </xsl:choose>
		...
		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	    ...
	    <!-- It's a cv index, and we want the value itself. -->
	    <xsl:when test="not($is-common) and
		...
		<xsl:otherwise>
		  <xsl:call-template name="expression">
		    ...
		    <xsl:with-param name="need-number-now" select="$need-number-now"/>
		    <xsl:with-param name="called-from"
				    select="'exp: recursing to resolve global cn'"/>
		    <xsl:with-param name="recursion" select="$recursion + 1"/>
		  </xsl:call-template>
		</xsl:otherwise>
	      </xsl:choose>
		...
		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
		...
		<xsl:call-template name="error-message">
		  <xsl:with-param name="msg">
		    <xsl:text>There is no gl to match "</xsl:text>
		    <xsl:value-of select="$valn"/>
		    <xsl:text>"</xsl:text>
		  </xsl:with-param>
		</xsl:call-template>
	      </xsl:if>
	      <!-- It's a number in gl/cn format, i.e. most likely a ref
		   to a pt in another gl. Recurse to resolve the ref. -->
	      <xsl:apply-templates select="key('gl-index',$gname)[1]"
		...
		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
		...
		<xsl:when test="$op != 'NaN'">
		  ...
		  <!-- A convenience var, true if the operator is 'index' -->
		  ...
		  <xsl:choose>
		    ...
		    <xsl:when test="$compile-now">
		      <xsl:choose>
			<xsl:when test="$operator = 'index' or $operator = 'pt'">
		    ...
		    -->
		    <!-- special handling for pt and index operators? -->
		    <xsl:when test="$operator = 'pt'"></xsl:when>
...
-->
  ...
  <xsl:template match="xgf:wpm" mode="push-me">
    ...
    <xsl:call-template name="expression">
      ...
      <xsl:with-param name="mp-container" select="$mp-container"/>
      <xsl:with-param name="to-stack" select="true()"/>
      <xsl:with-param name="called-from" select="'wpm push-me'"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="xgf:wpm" mode="get-my-val">
    ...
    <xsl:call-template name="expression">
      ...
      <xsl:with-param name="add-mode" select="$add-mode"/>
      <xsl:with-param name="expect" select="$expect"/>
      <xsl:with-param name="called-from" select="'wpm get-my-val'"/>
  ...
  <xsl:template match="xgf:wpm" mode="store-me">
  ...
  <!--
      ...
      cv-cut-in, single-width, single-width-cut-in,
      delta-base, delta-sh. Two others, round-state and
  ...
  <xsl:template name="store-value">
    ...
    <xsl:param name="all-mo-params"
	       select="ancestor::xgf:mo/xgf:pm|
		       ancestor::xgf:gl/xgf:pm"/>
    <xsl:param name="all-variables"
	       select="ancestor::xgf:gl/xgf:var|
		       ancestor::xgf:fn/xgf:var|
		       ancestor::xgf:prep/xgf:var"/>
    <xsl:param name="all-global-variables" select="/xgf:xgridfit/xgf:var"/>
    <xsl:param name="all-aliases"
	       select="$this-gl/xgf:alias|ancestor::xgf:fn/xgf:alias|
		       ancestor::xgf:mo/xgf:alias|
    ...
    <xsl:param name="all-global-aliases"
	       select="/xgf:xgridfit/xgf:alias"/>
    <xsl:choose>
      <xsl:when test="$all-aliases[@nm=$vname]">
	<xsl:call-template name="store-value">
	  <xsl:with-param name="vname" select="$all-aliases[@nm=$vname]/@target"/>
	  <xsl:with-param name="this-gl" select="$this-gl"/>
	  <xsl:with-param name="mp-container" select="$mp-container"/>
	  <xsl:with-param name="all-variables" select="$all-variables"/>
	  <xsl:with-param name="all-global-variables" select="$all-global-variables"/>
      ...
      <xsl:when test="$all-global-aliases[@nm=$vname]">
	<xsl:call-template name="store-value">
	  <xsl:with-param name="vname" select="$all-global-aliases[@nm=$vname]/@target"/>
	  <xsl:with-param name="this-gl" select="$this-gl"/>
	  <xsl:with-param name="mp-container" select="$mp-container"/>
	  <xsl:with-param name="all-variables" select="$all-variables"/>
	  <xsl:with-param name="all-global-variables" select="$all-global-variables"/>
...
<!-- *** params here *** -->
      <xsl:when test="$all-mo-params[@nm=$vname]">
	<xsl:variable name="this-id">
	  <xsl:call-template name="get-first-mp-id">
	    <xsl:with-param name="mp-container" select="$mp-container"/>
	  </xsl:call-template>
	...
	<xsl:choose>
	  <xsl:when test="string-length($this-id) and
			  $mp-containers and
			  $mp-containers[generate-id() = $this-id]/xgf:wpm[@nm=$vname]/@val">
	    <xsl:apply-templates
		select="$mp-containers[generate-id() = $this-id]/xgf:wpm[@nm=$vname]"
		mode="store-me"/>
	  </xsl:when>
	  <xsl:when test="$all-mo-params[@nm=$vname]/@val">
	    <xsl:call-template name="store-value">
	      <xsl:with-param name="vname" select="$all-mo-params[@nm=$vname]/@val"/>
	      <xsl:with-param name="this-gl" select="$this-gl"/>
	      <xsl:with-param name="mp-container" select="$mp-container"/>
	      <xsl:with-param name="all-mo-params" select="$all-mo-params"/>
	      <xsl:with-param name="all-variables" select="$all-variables"/>
...
<!-- End of params -->
      <xsl:when test="$vname = 'return' and ancestor::xgf:fn[@return = 'yes']">
      ...
      <xsl:when test="$all-variables[@nm=$vname]">
        <xsl:call-template name="store-var">
          <xsl:with-param name="val"
            select="$all-variables[@nm=$vname]"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="key('cvt',$vname)">
        <xsl:call-template name="store-cv">
      ...
      <!-- Any graphics var except for round-state and custom-round-state can be
      ...
      <xsl:when test="document('xgfdata.xml')/*/xgfd:var-locations/xgfd:loc[@name = $vname]">
        <xsl:choose>
          <xsl:when test="contains($vname,'default')">
            <xsl:choose>
              <xsl:when test="ancestor::xgf:prep">
              ...
              <xsl:otherwise>
                <xsl:call-template name="warning">
                  <xsl:with-param name="msg">
                    ...
                    <xsl:text>default graphics var </xsl:text>
                    <xsl:value-of select="$vname"/>
                    <xsl:value-of select="$newline"/>
                    <xsl:text>except in the &lt;prep&gt;. </xsl:text>
                    <xsl:text>The value this program has attempted</xsl:text>
                    <xsl:value-of select="$newline"/>
                    <xsl:text>to write to it is being discarded.</xsl:text>
                  </xsl:with-param>
      ...
      <xsl:when test="$all-global-variables[@nm=$vname]">
        <xsl:call-template name="store-global-var">
          <xsl:with-param name="val"
            select="$all-global-variables[@nm=$vname]"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="warning">
  ...
  <!-- 2. Handling fn parameters -->
  
  <!--

      push-fn-parameter

      This is always called from inside a fn; it is the way
      ...
      that, we started the fn by recording the stack depth, and
      now a little arithmetic will get us to where the parameters are
      stored.
  -->
  <xsl:template name="push-fn-parameter">
    <xsl:param name="val"/>
    <xsl:param name="this-fn" select="ancestor::xgf:fn"/>
    <xsl:param name="all-params" select="$this-fn/xgf:pm"/>
    ...
    <!-- The depth of the stack when the fn started is stored in
         var-fn-stack-count. -->
    <xsl:call-template name="number-command">
      <xsl:with-param name="num">
	<xsl:call-template name="resolve-std-var-loc">
    ...
    <!-- Get the position of the relevant pm element and add it to the
         number we just calculated. -->
    <xsl:call-template name="push-num">
      <xsl:with-param name="num"
        select="count($all-params[@nm = $val]/preceding-sibling::xgf:pm) + 1"/>
    </xsl:call-template>
    <xsl:call-template name="simple-command">
  ...
  <!-- "Variables" are really locations in the TT Storage Area. We
       ...
       variables; above that is a "var frame" (defined by
       var-frame-bottom and var-frame-top), which moves up and down as
       the scope changes. Result is that every gl program and every
       fn (also the prep) gets its own private space in
       the Storage Area. If you call a fn recursively, each
       iteration gets its own set of variables. -->
  
  <!-- Call at the beginning of each gl program and the prep
       to set up the var frame. Code is inserted only if the
       program contains variables or calls a fn (we don't try to
       predict whether the fn will contain variables). -->
  <xsl:template name="set-up-var-frame">
    <xsl:call-template name="push-num">
      <xsl:with-param name="num">
	<xsl:call-template name="resolve-std-var-loc">
    ...
    <xsl:call-template name="push-num">
      <xsl:with-param name="num">
	<xsl:call-template name="resolve-std-var-loc">
	  <xsl:with-param name="n" select="$var-frame-base"/>
	</xsl:call-template>
      </xsl:with-param>
    ...
    <xsl:choose>
      <xsl:when test="xgf:var">
	    ...
	    <xsl:call-template name="resolve-std-var-loc">
	      <xsl:with-param name="n" select="$var-frame-base"/>
	      <xsl:with-param name="add" select="count(xgf:var)"/>
	    </xsl:call-template>
	  </xsl:with-param>
        </xsl:call-template>
      </xsl:when>
  ...
  <!--
      ...
      Start with a var element; get its position in the variables
  ...
  <xsl:template name="push-var">
    ...
    <xsl:call-template name="number-command">
      <xsl:with-param name="num" select="count($val/preceding-sibling::xgf:var) + 1"/>
      <xsl:with-param name="cmd" select="'ADD'"/>
    </xsl:call-template>
  ...
  <xsl:template name="push-global-var">
    <xsl:param name="val"/>
    <xsl:param name="index-only" select="false()"/>
    <xsl:call-template name="push-num">
      <xsl:with-param name="num">
	<xsl:call-template name="resolve-std-var-loc">
	  ...
	  <xsl:with-param name="add" select="count($val/preceding-sibling::xgf:var) + 1"/>
	</xsl:call-template>
      </xsl:with-param>
    </xsl:call-template>
    <xsl:if test="not($index-only)">
  ...
  <xsl:template name="store-global-var">
    <xsl:param name="val"/>
    <xsl:call-template name="push-num">
      <xsl:with-param name="num">
	<xsl:call-template name="resolve-std-var-loc">
	  ...
	  <xsl:with-param name="add"
			  select="count($val/preceding-sibling::xgf:var) + 1"/>
	</xsl:call-template>
      </xsl:with-param>
    </xsl:call-template>
    <xsl:call-template name="simple-command">
  ...
  <xsl:template match="xgf:var" mode="initialize">
	...
	<xsl:with-param name="to-stack" select="true()"/>
      ...
      <xsl:call-template name="store-value">
	<xsl:with-param name="vname" select="@nm"/>
      </xsl:call-template>
    </xsl:if>
...
<!--
	...
	<xsl:with-param name="mp-container"
		      select="$mp-container"/>
	<xsl:with-param name="called-from" select="'x-store-cv 1'"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:if test="$cvt-index = 'NaN'">
      <xsl:call-template name="error-message">
	<xsl:with-param name="msg">
	  <xsl:text>Can't resolve cv "</xsl:text>
...
-->
  ...
  <xsl:template name="push-homogeneous-list">
	...
	<xsl:with-param name="is-pt-list" select="$is-pt-list"/>
	<xsl:with-param name="permitted" select="$permitted"/>
	<xsl:with-param name="mp-container" select="$mp-container"/>
  ...
  <xsl:template name="push-list">
    ...
    <xsl:param name="is-pt-list" select="false()"/>
