...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:when test="$all-defaults[@type=$var-name] and
        ...
        <xsl:call-template name="set-simple-graphics-var">
          <xsl:with-param name="value"
                          select="$all-defaults[@type=$var-name]/@val"/>
  ...
  <xsl:template name="prep-instructions">
    <xsl:param name="all-defaults" select="/xgf:xgridfit/xgf:default"/>
    ...
    <xsl:if test="xgf:var or descendant::xgf:callf">
    ...
    <!-- Set up some defaults. Either a default has been specified in the
         ...
         default because we're calling from the prep, or we just
      ...
      <xsl:when test="$all-defaults[@type='round-state']">
        <xsl:call-template name="do-set-round">
          <xsl:with-param name="round-state"
            select="$all-defaults[@type='round-state']/@val"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="push-num">
    ...
    <xsl:call-template name="initialize-graphics-var">
      <xsl:with-param name="var-name" select="'cv-cut-in'"/>
      <xsl:with-param name="loc">
	<xsl:call-template name="resolve-std-var-loc">
    ...
    <xsl:choose>
      <xsl:when test="$all-defaults[@type =
                      'delta-sh'] and not($use-tt-defaults)">
        <xsl:variable name="n">
    ...
    <!-- DONE WITH GRAPHICS STATE.
        Now process other instructions in the prep. -->
    <xsl:apply-templates/>
    <!-- Any functions to define in prep? -->
    <xsl:apply-templates select="/xgf:xgridfit/xgf:fn[xgf:variant]"/>
  </xsl:template>
</xsl:stylesheet>
