...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="do-mirp">
    ...
    <xsl:if test="not($suppress-set-rp0) and $ref-pt">
      <!-- Set RP0 if a ref pt has been specified. ZP0 for this pt. -->
    ...
    <xsl:call-template name="push-pt-and-distance">
      <xsl:with-param name="pt" select="$move-point"/>
  ...
  <!--
       ...
       Similar to "do-mirp" but there's no ref pt, no minimum distance,
  ...
  <xsl:template name="do-mdrp">
    ...
    <xsl:if test="not($suppress-set-rp0) and $ref-pt">
      <!-- Set RP0 if a ref pt has been specified. -->
      <xsl:call-template name="push-pt">
        <xsl:with-param name="pt" select="$ref-pt"/>
        <xsl:with-param name="zp" select="'0'"/>
	<xsl:with-param name="mp-container"
    ...
    <!-- Push the pt we're going to move. -->
    <xsl:call-template name="push-pt">
      <xsl:with-param name="pt" select="$move-point"/>
      <xsl:with-param name="zp" select="'1'"/>
      <xsl:with-param name="mp-container"
  ...
  <xsl:template name="do-mdap">
    ...
    <!-- Push the pt we're going to move. ZP0 for this pt. -->
    <xsl:call-template name="push-pt">
      <xsl:with-param name="pt" select="$move-point"/>
      <xsl:with-param name="zp" select="'0'"/>
      <xsl:with-param name="mp-container"
  ...
  <xsl:template name="do-alignrp">
    ...
    <xsl:if test="$move-points">
      <xsl:call-template name="push-points">
        <xsl:with-param name="pts" select="$move-points"/>
        <xsl:with-param name="zp" select="'1'"/>
	<xsl:with-param name="mp-container"
			select="$mp-container"/>
      </xsl:call-template>
      <xsl:if test="count($move-points) &gt; 1">
        <xsl:call-template name="number-command">
          <xsl:with-param name="num" select="count($move-points)"/>
      ...
      <xsl:if test="$move-points[1]/@zone">
