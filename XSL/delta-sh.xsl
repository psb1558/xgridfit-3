  ...
  <xsl:template name="exec-delta">
	...
	<xsl:choose>
	  <xsl:when test="$caller-is='pt-delta'">
		...
		<xsl:value-of select="./xgf:pt/@n"/>
	      </xsl:when>
	      <xsl:when test="parent::xgf:delta/xgf:pt">
		<xsl:value-of select="parent::xgf:delta/xgf:pt/@n"/>
	      </xsl:when>
	      <xsl:when test="ancestor::xgf:mv">
		<xsl:value-of select="ancestor::xgf:mv[1]/xgf:pt/@n"/>
	      </xsl:when>
	      <xsl:otherwise>
		<xsl:call-template name="error-message">
		  <xsl:with-param name="msg">
		    Cannot find a pt for &lt;delta-set&gt;.
		  </xsl:with-param>
		</xsl:call-template>
	      </xsl:otherwise>
	    </xsl:choose>
