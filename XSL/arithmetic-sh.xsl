...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="xgf:cv-index">
    <xsl:param name="mp-container"/>
    <xsl:call-template name="debug-start"/>
    <xsl:variable name="r" select="@result-to"/>
    <xsl:if test="not(ancestor::*/xgf:var[@nm=$r])">
      <xsl:call-template name="error-message">
        <xsl:with-param name="msg">
          <xsl:text>The "result-to" attribute in a &lt;cv-index&gt;</xsl:text>
          <xsl:text> instruction</xsl:text>
          <xsl:value-of select="$newline"/>
          <xsl:text>must be the name of a var.</xsl:text>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:if>
    <xsl:call-template name="expression">
