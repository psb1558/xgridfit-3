...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:if test="$value or number($value) = 0 or $stack-source-permitted">
      ...
      <xsl:if test="$save">
        ...
        <xsl:if test="$default-loc and
                      ancestor-or-self::xgf:prep and
                      $may-record-default and
                      not($all-defaults[@type='use-truetype-defaults']/@val = 'yes')">
  ...
  <!--
      ...
      "gl"; it specifies the zone to which we are setting the
  ...
  <!--
      ...
      set-zone-pointers-to-gl

      A convenience: we set all zone pointers to the gl
      zone at the end of any procedure that might have
      changed one or more of them.
  -->
  <xsl:template name="set-zone-pointers-to-gl">
  ...
  <xsl:template name="resolve-delta-sh-value">
    <xsl:param name="v"/>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:call-template name="error-message">
          <xsl:with-param name="msg">
            <xsl:text>Attempting to set delta sh: illegal value "</xsl:text>
  ...
  <xsl:template name="do-set-round">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$all-round-states[@nm=$round-state]">
        <xsl:call-template name="do-super-round">
          <xsl:with-param name="period"
                          select="$all-round-states[@nm
                                  = $round-state]/@period"/>
	  ...
	  -->
		...
		<xsl:with-param name="n" select="$var-sround-info"/>
          ...
          <xsl:if test="ancestor-or-self::xgf:prep and $may-record-default and
                        not($all-defaults[@type='use-truetype-defaults']/@val = 'yes')">
	    <!--
  ...
  <xsl:template match="xgf:setvs">
    <xsl:param name="mp-container"/>
    <xsl:call-template name="debug-start"/>
    <xsl:call-template name="do-setvs">
  ...
  <xsl:template match="xgf:setcv">
    <xsl:param name="mp-container"/>
    <xsl:call-template name="x-store-cv">
      <xsl:with-param name="cvtname" select="@nm"/>
  ...
  <xsl:template match="xgf:set-minimum-distance | xgf:set-cv-cut-in |
		       xgf:set-single-width-cut-in | xgf:set-single-width |
		       xgf:set-delta-base | xgf:set-delta-sh">
    <xsl:param name="mp-container"/>
    <xsl:call-template name="debug-start"/>
    <xsl:variable name="ln" select="substring(local-name(),5)"/>
    <xsl:variable name="v">
      <xsl:choose>
	<xsl:when test="$ln = 'delta-sh'">
	  <xsl:call-template name="resolve-delta-sh-value">
  ...
  <xsl:template match="xgf:with-minimum-distance | xgf:with-control-value-cut-in |
		       ...
		       xgf:with-delta-base | xgf:with-delta-sh">
    <xsl:param name="mp-container"/>
    <xsl:call-template name="debug-start"/>
  ...
  <xsl:template name="restdef">
    ...
    <xsl:choose>
      <xsl:when test="$entry/@nm = 'round-state'">
	<xsl:call-template name="storage-to-storage">
	  <xsl:with-param name="src">
	    <xsl:call-template name="resolve-std-var-loc">
	      <xsl:with-param name="n" select="$var-round-state-default"/>
