...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="gl-list" xmlns:excom="http://exslt.org/common">
    <xsl:param name="separator" select="' '"/>
    <xsl:param name="list"/>
    <xsl:variable name="s" select="normalize-space($list)"/>
    <xsl:variable name="this-gl">
      <xsl:choose>
	...
	<xsl:apply-templates select="key('gl-index',$this-gl)"/>
      </xsl:when>
      <xsl:otherwise>
	<xsl:variable name="new-filename"
		      select="concat($outfile_base,'_',$this-gl,$file-extension)"/>
	<xsl:choose>
